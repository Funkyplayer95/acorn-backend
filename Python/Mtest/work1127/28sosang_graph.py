...
# matplotlib대신 seaborn연결 
plt.figure(figsize=(10, 6))
plt.bar(cnt7, cnt5)
plt.title('hbi 소상공인시장진흥공단_상가(상권)정보_서울_202109 11-28-화요일')
plt.show()


plt.figure(figsize=(10, 6))
# plt.bar(cnt7, cnt5)
sns.barplot(x=cnt5, y=cnt7)
plt.title('hbi 소상공인시장진흥공단_상가(상권)정보_서울_202109 sns그래픽')
plt.show()
...
print(star) #[491 rows x 39 columns]  원본데이터 #[325,880 rows x 39 columns]
print()
latlong = star[['위도','경도']]
mc_cluster = MarkerCluster().add_to(m) #기본위도경도를 서울시청=덕수궁근처 좌표지정
for lat, long in zip(latlong['위도'], latlong['경도']):
    folium.Marker([lat,long], icon=folium.Icon(color='red')).add_to(mc_cluster)


m.save('./data/sosang25.html')
webbrowser.open('file://'+os.path.realpath('./data/sosang25.html'))
print()
print('-' * 100)
