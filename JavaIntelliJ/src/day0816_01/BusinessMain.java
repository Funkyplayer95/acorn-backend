...
public class BusinessMain {
	...
	public static int goodStockProcess(int iCurrentStock, int iProperStock, Scanner sc) {
		// 현재재고와 적정재고 확인
		// 현재재고가 적정재고보다 작은지
		System.out.println("보충전 현재 재고 : " + iCurrentStock);
		...
		System.out.println("보충후 현재 재고 : " + iCurrentStock );
		System.out.print("주문 하시겠습니까?? (Y/N) ");
		String sInputValue = sc.nextLine();
		if(sInputValue.equalsIgnoreCase("y")){

			iCurrentStock = userOrderProcess(sc);
		}
		else if(sInputValue.equalsIgnoreCase("n")){
			System.exit(0);
		}
		// 사용자 주문 시뮬레이션(랜덤을 사용)
...
//		iCurrentStock -= iCustomerOrderQty;
	...
	public static int userOrderProcess(Scanner sc) {
		int iCurrentStock =0;
