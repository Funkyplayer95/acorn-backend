...
def crawl():
    ...
    for i in range(5000):
        try :
            ...
            try:
                with open('tiktok_random.csv', 'r'):
                    pass
            except FileNotFoundError:
                columns_name = ['Writer', 'Content', 'Date', 'Like', 'Tag', 'Comments', 'Url']
                with open('tiktok_random.csv', 'a', encoding='utf-8-sig', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow(columns_name)
            finally:
                if check_value_in_column('tiktok_random.csv','Url',objListup[6]):
                    print('중복')
                    continue
                else :
                    with open('tiktok_random.csv', 'a', encoding='utf-8-sig', newline='') as file:
