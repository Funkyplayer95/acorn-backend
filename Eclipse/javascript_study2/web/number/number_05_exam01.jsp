...
function radixConvert() {
	...
	if( (sv == "") || (sv == null) ) {
		...
		if(typeof(sv) == "string") {  // number, boolean
			var cv = new Number(sv);
			var ret = confirm('문자열입니다. 변환하시겠습니까?');
			if(ret == true) {
				if(radixTxt == "2") {
					document.getElementById('convertedVal').value = cv.toString(2); // (), (2 or 16)
				} else if(radixTxt == "16") {
			...
			if(radixTxt == "2") {
				document.getElementsByName('convertedVal')[0].value = cv.toString(2);
			} else if(radixTxt == "16") {
				document.getElementsByName('convertedVal')[0].value = cv.toString(16);
			} else {
				alert("지원하지 않는 진수를 선택하셨습니다.");
				return;
			}			
...
<table border="1">
	...
	<option value="32">32진수</option></select>
	</td>
	<td><input type="text" name="convertedVal" id="convertedVal" /></td></tr>	
	<tr><td colspan="2" bgcolor="green"><input type="button" value="변환" 
	onclick="radixConvert()" /></td></tr>
</table>
</body>
</html>